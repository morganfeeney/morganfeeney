Frontend developer, design engineer, and product builder based in Manchester, UK.
I've spent the last decade building websites and products that generate leads, sales, and recurring revenue.

Some stuff I'm working on:

- 🎨 [shadcnpreset](https://shadcnpreset.com): Browse, compare, and discover thousands of shadcn/ui theme presets.
- ⚡️ [ZippyStarter](https://zippystarter.com): A suite of production-ready Next.js SaaS starter kits, templates and component library.
- 🎨 [shadcn theme generator](https://zippystarter.com/tools/shadcn-ui-theme-generator): Generate custom shadcn/ui themes in seconds.
- 📸 [OG Image](https://ogimage.info) — Quickly validate Open Graph images and social sharing previews.

## Tech

Next.js • React • TypeScript • Tailwind CSS • shadcn/ui • Design Systems • Accessibility • SEO

## Freelance

I take on freelance work, helping businesses build SaaS products, e-commerce stores, marketing websites, landing pages, and design systems. If I'm unavailable, I can usually recommend someone from my network.

- 👉 [About me](https://morganfeeney.com/about)
- 👉 [Contact](https://morganfeeney.com/contact)
- ☕️ [Buy me a coffee](https://buymeacoffee.com/morganf)
- 🤑 [Sponsor me](https://github.com/sponsors/morganfeeney)
